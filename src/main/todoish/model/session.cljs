...
(defn logout [env]
  (-> env
    ...
    (uism/trigger-remote-mutation :actor/login-form 'todoish.model.session/logout {})
    (uism/activate :state/logged-out)))

(defn login [{::uism/keys [event-data] :as env}]
  (-> env
    (clear)
    (uism/trigger-remote-mutation :actor/login-form 'todoish.model.session/login
...
(defn signup-class [] (comp/registry-key->class :todoish.ui.root/Signup))
